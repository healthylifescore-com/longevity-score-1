...
-- Ensure profiles table has proper constraints for data integrity
ALTER TABLE public.profiles 
ADD CONSTRAINT IF NOT EXISTS profiles_email_not_empty 
CHECK (length(trim(email)) > 0);

-- Add constraint to ensure quiz responses have valid email format
ALTER TABLE public.quiz_responses 
ADD CONSTRAINT IF NOT EXISTS quiz_responses_email_format 
